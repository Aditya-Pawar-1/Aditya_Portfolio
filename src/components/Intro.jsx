...
const Intro = () => {
  ...
  useGSAP(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      gsap.to(main, { rotationY: xNorm * 10, rotationX: -yNorm * 10, transformPerspective: 1000, duration: 0.5, ease: "power3.out" });
  ...
  return (
    <section ref={root} className="relative min-h-[100vh] w-full overflow-hidden text-white perspective-[1000px]">
      <div ref={bgRef} className="absolute inset-0 z-0 scale-105">
        <div className="w-full h-full bg-[url(/assets/images/black-painted-wall-textured-background.jpg)] bg-no-repeat bg-top bg-cover opacity-40" />
      </div>
        ...
        <div className="w-full lg:w-1/2 space-y-8 z-20">
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              <p ref={labelRef} className="uppercase tracking-[0.25em] text-xs font-medium text-indigo-500">
                who am i?
              </p>
              <div ref={lineRef} className="h-[1px] w-24 bg-indigo-500" />
            </div>

            <div ref={headingBlockRef} onMouseMove={handleHeadingMove} onMouseLeave={handleHeadingLeave} className="select-none group cursor-pointer perspective-[1000px] will-change-transform">
          ...
          <div className="inline-block p-2 -m-2 rounded-lg transition-colors hover:bg-white/5 cursor-default" onMouseMove={handleSubheadingMove} onMouseLeave={handleSubheadingLeave}>
            <p ref={subheadingRef} className="max-w-lg text-sm md:text-base text-slate-400 leading-relaxed will-change-transform">
              I bridge the gap between <span className="text-white font-medium">design</span> and <span className="text-white font-medium">engineering</span>. I build pixel-perfect, engaging, and accessible digital experiences using modern architecture.
            </p>
          </div>
        </div>

        <div className="w-full lg:w-1/2 relative flex items-center justify-center h-auto mt-12 lg:mt-0 lg:h-auto">
          <div ref={visualContainerRef} className="relative w-[300px] md:w-[400px] aspect-square flex items-center justify-center cursor-pointer">
            <div ref={glowRef} className="absolute inset-0 bg-indigo-600/20 blur-[80px] rounded-full pointer-events-none" />

            <div ref={floatCard1Ref} className="absolute -top-6 -left-6 md:-left-12 z-30 w-32 md:w-40 p-3 rounded-xl border border-white/10 bg-black/40 backdrop-blur-md shadow-2xl">
            ...
            <div ref={floatCard2Ref} className="absolute -bottom-8 -right-4 md:-right-10 z-30 w-32 h-32 rounded-full border border-white/10 bg-white/5 backdrop-blur-xl flex items-center justify-center shadow-2xl">
